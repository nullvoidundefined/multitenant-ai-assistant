...
describe('orgs repository', () => {
  ...
  describe('createOrg', () => {
    it('creates org, adds creator as admin, and creates assistant config', async () => {
      const org = { id: 'org-1', name: 'Test Org', slug: 'test-org-abc', created_at: new Date(), updated_at: null };
      vi.mocked(withTransaction).mockImplementation(async (fn) => {
        const client = {} as never;
        return fn(client);
      });
      vi.mocked(query)
        .mockResolvedValueOnce({ rows: [org], rowCount: 1, command: '', oid: 0, fields: [] })
  ...
  describe('getOrgById', () => {
    it('returns org when found', async () => {
      const org = { id: 'org-1', name: 'Test', slug: 'test', created_at: new Date(), updated_at: null };
      vi.mocked(query).mockResolvedValue({ rows: [org], rowCount: 1, command: '', oid: 0, fields: [] });
  ...
  describe('getUserOrgs', () => {
    it('returns orgs with role for user', async () => {
      const orgs = [
        { id: 'org-1', name: 'Org 1', slug: 'org-1', role: 'admin', created_at: new Date(), updated_at: null },
      ];
      vi.mocked(query).mockResolvedValue({ rows: orgs, rowCount: 1, command: '', oid: 0, fields: [] });
  ...
  describe('getMembership', () => {
    it('returns membership when user belongs to org', async () => {
      const membership = { org_id: 'org-1', user_id: 'u1', role: 'admin', joined_at: new Date() };
      vi.mocked(query).mockResolvedValue({ rows: [membership], rowCount: 1, command: '', oid: 0, fields: [] });
  ...
  describe('listMembers', () => {
    it('returns members for org', async () => {
      const members = [
        ...
        { user_id: 'u2', email: 'c@d.com', role: 'member', joined_at: '2024-01-02' },
      ];
      vi.mocked(query).mockResolvedValue({ rows: members, rowCount: 2, command: '', oid: 0, fields: [] });
  ...
  describe('getAssistantConfig', () => {
    it('returns config when it exists', async () => {
      const config = { system_prompt: 'You are a bot.', model: 'claude-sonnet-4-20250514', max_tokens: 4096, temperature: 0.7 };
      vi.mocked(query).mockResolvedValue({ rows: [config], rowCount: 1, command: '', oid: 0, fields: [] });
